...
if Menu.items and Menu.items.has_key('mantis_main'):
    ...
    if authoring_menu:
        authoring_menu.children += (MenuItem("Campaign Indicators", reverse('url.mantis_authoring.transformers.stix.campaing_indicators'), check = lambda request: request.user.is_authenticated()),)
